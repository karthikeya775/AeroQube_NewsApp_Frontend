 ...
 const [articleStats, setArticleStats] = useState({
  ...
  useEffect(() => {
    ...
    const stats = {
      ...
      draft: submissions.filter(article => article.status === 'draft').length
    };
  ...
  const calculatePerformanceMetrics = () => {
    ...
    const avgWordCount = submissions.length > 0
      ? Math.round(submissions.reduce((acc, curr) => 
          acc + curr.content.split(' ').filter(word => word.trim() !== '').length, 0
        ) / submissions.length)
      : 0;
    ...
    const monthlyCount = submissions.filter(
      article => new Date(article.submittedDate) > thirtyDaysAgo
    ).length;
